export class GuardGroup extends HTMLElement
{
  constructor() {
    super();
    if ('' != this.innerHTML.trim()) {
      return;
    }
    this.classList.add('guard-group');
    let uuid = this.uuidv4();
    this.innerHTML = '<input type="checkbox" id="' + uuid + '"><label for="' + uuid + '">&nbsp;</label>'
    let checkboxs = this.getElementsByTagName('input');
    this.checkbox = checkboxs[0];
    this.token = this.dataset.token;
    this.checkbox.addEventListener('change', this.onChange.bind(this));
    this.changeState();
    let observer = new MutationObserver(this.mutationObserver.bind(this));
    observer.observe(this, {
      attributes: true
    });
  }

  mutationObserver(mutations) {
    mutations.forEach(this.forEachMutationObserver.bind(this));
  }

  forEachMutationObserver(mutation)
  {
    if ('attributes' === mutation.type && 'data-state' === mutation.attributeName) {
      this.changeState();
    }
  }

  changeState()
  {
    this.checkbox.checked = ("1" === this.dataset.state) ? true : false;
  }


  onChange(element) {
    element.preventDefault();
    const urlSearchParams = new URLSearchParams();
    urlSearchParams.append('_token', this.dataset.url);
    urlSearchParams.append('state', this.checkbox.checked);
    let options = {
      method: 'POST',
      headers: {
        "Content-type": "application/x-www-form-urlencoded; charset=UTF-8",
      },
      body: urlSearchParams,
    };
    fetch(this.dataset.url, options).catch((err) => {
      console.log(err);
    });
  }

  uuidv4() {
    return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
      var r = Math.random() * 16 | 0, v = c == 'x' ? r : (r & 0x3 | 0x8);
      return v.toString(16);
    });
  }
}